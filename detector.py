...
def get_risk_level(score):
    if score >= 40:
        ...
        return "low"
def get_fraud_patterns(signals):
    patterns = []
    if signals["velocity"]:
        patterns.append("velocity")
    if signals["ip_reputation"]:
        patterns.append("ip_reputation")
    if signals["geo"]:
        patterns.append("geo_anomaly")
    if signals["anomaly"]:
        patterns.append("ml_anomaly")
    if signals["unique_ips"] > 1:
        patterns.append("multiple_ips")
    ...
    for card, group in grouped:
        ...
        if risk >= 15:
            suspicious_cards.append({
                ...
                "unique_ips": unique_ips,
                "risk_score": risk,
                # FIX: these fields were missing — script.js references both
                "risk_level": get_risk_level(risk),
                "fraud_patterns": get_fraud_patterns(signals)
            })
    propagate_risk(G)
